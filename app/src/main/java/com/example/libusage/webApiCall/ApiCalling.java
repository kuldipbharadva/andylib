...
//                                CustomDialog.dismissProgress(context);
    ...
    public static void makeApiCall(final Context context, final String url, final String method, final Map headerMap, final Object requestBody, final boolean isProgressDialogNeeded, final boolean isGetResponse, final ApiSuccessInterface apiSuccessInterface) {
        Log.d(TAG, "makeApiCall: request :: " + new Gson().toJson(requestBody));
        Log.d(TAG, "makeApiCall: header  :: " + new Gson().toJson(headerMap));
        Log.d(TAG, "makeApiCall: req url :: " + url);
        try {
